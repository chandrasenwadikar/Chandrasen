...
negative_vocab=['bad','terriable','useless','hate',':(']
netural_vocab=['movie','the','sound','was','is','as','it','across','did','know','words','not']
    
positive_features=[(word_feats(word),'pos')for word in positive_vocab]
negative_features=[(word_feats(word),'neg')for word in negative_vocab]
netural_fetaures=[(word_feats(word),'neu')for word in netural_vocab]

print(poisitve_vocab)
print(negative_vocab)
print(netural_vocab)


train_set=negative_features+positive_features+netural_features
print(train_set)


classifier=NavieBayesClassifier.train(train_set)



def sentiment_res(sentence):
    neg=0
    pos=0
...
sentence = "horrible movie,I hated it"

pos,neg=sentiment_res(sentence)
...
from textblob import TextBlob # Text blob in built function for sentiment analysis
def get_sentiment(sentence):
    analysis=TextBlob(sentence)
    if analysis.sentiment.polarity >0:
        return 'positive',analysis.sentiment.polarity
    elif analysis.sentiment.polarity ==0:
        return 'netural', analysis.sentiment.polarity
    else:

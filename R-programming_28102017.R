...
#read.csv("C:/Users/Administrator/Desktop/HospitalCosts.csv")

#hospitaltest <-lm(TOTCHG ~AGE+FEMALE+LOS+RACE+APRDRG,data =HospitalCosts)
...
View(Hos_cost)
Model_HC<-lm(TOTCHG ~LOS+APRDRG,data = Hos_cost)
summary(Hos_cost)
test<-data.frame(Hos_cost$APRDRG)
pretest<-predict(Hos_cost)
filter(test$APRDRG)


cor(Hos_cost)#Corelation matrix


###Logistics Regression
install.packages('readxl')
library(readxl)
readtest<-read.csv("C:/Users/Administrator/Desktop/CreditRisk.csv")
crt<-CreditRisk1
View(CreaditRisk1)
View(CreditRisk1)
CreditRisktest<-read.csv("CreditRisk1.csv")
View(CreditRisk1)

install.packages('dplyr')
library(dplyr)
CreditRisk<-mutate(CreaditRisk,Gender1=ifelse(Gender=='Male',0,1))
CreditRisk<-mutate(CreaditRisk,Married1=ifelse(Married=='Yes',0,1))
CreditRisk<-mutate(CreaditRisk,Education1=ifelse(Education=='Graduate',0,1))
CreditRisk<-mutate(CreaditRisk,Self_Employed1=ifelse(Self_Employed=='Yes',0,1))
CreditRisk<-mutate(CreaditRisk,Property_Area1=ifelse(Property_Area=='Urban',0,1))
CreditRisk<-mutate(CreaditRisk,Loan_Status1=ifelse(Loan_Status=='N',0,1))

Credit_Risk<-read.csv("CreditRisk.csv")
dim(creditrisk);View(creditrisk);summary(creditrisk)

table(CreditRisk$Loan_Status,CreditRisk$Credit_History)



###

Credit_Risk <-mutate(Credit_Risk,newarea=ifelse(Property_Area=="Urban",s,ifelse(Property_Area=="Rural",0,1)))
CreditRisk<-select(CreditRisk,4,7,8,4,3)

factor(CreditRisk$Loan_Status1)

factor(CreditRisk$Loan_Status1)
factor(CreditRisk$Loan_Status1)

CreditRisk$Loan_Status1<-factor(CreditRisk$Loan_Status1);is.factor(CreditRisk$Loan_Status1)

dim(CreaditRisk)


sampletest <-sample(2,nrow(CreditRisk),replace=TRUE,prob=c(.8,.2))
Train_smapletest <-CreditRisk[sampletest ==1,]
Test_sampletest <-CreditRisk[sampletest ==2,]
View(CreaditRisk)

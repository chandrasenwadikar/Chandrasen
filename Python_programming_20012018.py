...
#Data frame- is a two dimensional structure
...
data2=[{'a':1,'b':2,'c':3},{'a':5,'b':10,'c':9}]
df2=pd.DataFrame(data2,index=['first','second'],columns=['a','b','c'])
print(df2)
#append the data frame(slicing and dicing the data set)
df=df.append(df2)
print(df)
...
d={'Name':pd.Series(['Tom','James','Ricky','Vin']),'Age':pd.Series([24,66,21,34]),'Rating':pd.Series([4,3,2,4.8])}
df=pd.DataFrame(d)
df
...
df.to_csv("C:\\Users\\Admin\\Desktop\\myfirstfile.csv",index=False) # my first file not created by manually, it is automatically created by pandas
...
#trying to read a file that dosent exist
...
stud1.displayStudent(X)
stud2.displayStudent(Y)

#################


print("Hello World"[::-1]) #Reverseing the string
import numpy as np

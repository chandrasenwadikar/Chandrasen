...
model_AP1111<-arima(AirPassengers,c(1,1,1))
model_AP222<-arima(AirPassengers,c(2,2,0))
model_AP333<-arima(AirPassengers,c(2,1,1))
model_AP_444<-arima(AirPassengers,c(1,2,2))
model_AP_555<-arima(AirPassengers,c(4,2,4))


model_air_predict1<-predict(model_AP_555,n.ahead =5) # n.ahead = 5 means how many iteration we want to predict, here it is 5 times
...
week(ymd("2020-12-11"))#Gives week of that year 
day(ymd("2020-12-11"))#Day of the month
month(ymd("2020-12-11"))#month of the year

date1<-ymd("2015-01-01")
date2<-ymd("2016-01-01")
